    ...
    def find_kernel(self, dataset, alpha):
        ...
        if cn(dataset, alpha):
            ...
            if self.div_conq_expand:
                ...
                logging.debug(f"Dataset expanded using divide-and-conquer: {expanded_dataset.get_elements()}")
            else:
                expanded_dataset = expand(dataset, alpha, self.sw_expand)
                logging.debug(f"Dataset expanded using sliding window of size {self.sw_expand}: {expanded_dataset.get_elements()}")

            # Shrink phase
            if self.div_conq_shrink:
                kernel = shrink_divide_and_conquer(expanded_dataset, alpha)
                logging.debug(f"Dataset shrunk using divide-and-conquer: {kernel.get_elements()}")
            else:
                if self.sw_shrink > 1:
                    kernel = shrink_sliding_window(expanded_dataset, alpha, self.sw_shrink)
                    logging.debug(f"Dataset shrunk using sliding window of size {self.sw_shrink}: {kernel.get_elements()}")
                else:
                    kernel = shrink(expanded_dataset, alpha)
                    logging.debug(f"Dataset shrunk using basic shrink: {kernel.get_elements()}")
            return kernel
        else:
            logging.debug("Alpha is not a consequence of the dataset. Returning None.")
            return None
