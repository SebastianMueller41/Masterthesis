...
def expand(B_dataset, alpha, window_size=1):
    ...
    for start in range(0, len(elements), window_size):
        ...
        if counter > len(elements) + 5:
            kr_logger.warning("BREAK, MAX ITERATION, RETURNING NONE.")
            return None
        window_end = min(start + window_size, len(elements))
        window_elements = elements[start:window_end]
        for element in window_elements:
            B_prime.add_element(element)
        kr_logger.debug(f"B_PRIME with {len(B_prime.get_elements())} elements: {B_prime.get_elements()}")
        if cn(B_prime, alpha):
            return B_prime
    return B_prime
...
def expand_divide_and_conquer(B_dataset, alpha, removed_elements=None):
    ...
    kr_logger.info(f"DC_EXPAND: Splitting removed elements: {removed_elements.get_elements()}")
    ...
    kr_logger.info(f"DC_EXPAND: Left half: {left_removed.get_elements()} entails alpha: {cn_left}, Right half: {right_removed.get_elements()} entails alpha: {cn_right}")

    if cn_left:
        kr_logger.info(f"D&C: Left half added to B, call D&C again with right half!")
        return expand_divide_and_conquer(left_expanded, alpha, right_removed)
    elif cn_right:
        kr_logger.info(f"D&C: Right half added to B, call D&C again with left half!")
...
def shrink(B_dataset, alpha):
    ...
    while i < len(B_dataset.get_elements()):
        if current_iteration > max_iterations:
            kr_logger.error("Forced break for debugging.")
            break
        current_iteration += 1
        element = B_dataset.get_elements()[i]
        kr_logger.info(f"Checking line: {element} with index: {i}")
        cloned_B_dataset = B_dataset.clone()
        kr_logger.debug(f"Removing element: {element}")
        cloned_B_dataset.remove_element(element)
        kr_logger.debug(f"B with {len(cloned_B_dataset.get_elements())} elements = {cloned_B_dataset.get_elements()}")
        if cn(cloned_B_dataset, alpha):
            kr_logger.info(f"SHRINK: CN = TRUE, removing: {element}")
            B_dataset.remove_element(element)
            kr_logger.debug(f"CONTINUE SHRINKING WITH {len(B_dataset.get_elements())} elements : {B_dataset.get_elements()}")
        else:
            i += 1
    kr_logger.info(f"Kernel output with {len(B_dataset.get_elements())} elements: {B_dataset.get_elements()}")
    kr_logger.info("KERNEL BLACKBOX FINISHED")
    return B_dataset
...
def shrink_sliding_window(B_dataset, alpha, window_size):
    ...
    while start < len(elements):
        ...
        if not cn(B_prime, alpha):  # Use the imported cn function
            kr_logger.info(f"SHRINK: CN = FALSE for window elements {window_elements}, B_dataset = {B_prime.get_elements()}, removed elements: {removed_elements.get_elements()}")
            start += window_size  # Step size is window_size
        else:
            kr_logger.info(f"CN = TRUE for window elements {window_elements}. Deleting these elements.")
            B_dataset = B_prime
            for element in window_elements:
...
def shrink_divide_and_conquer(B_dataset, alpha):
    ...
    kr_logger.info(f"DC splitting Dataset: {B_dataset.get_elements()}")
    B1, B2 = B_dataset.split()

    cn_B1 = cn(B1, alpha)
    cn_B2 = cn(B2, alpha)

    kr_logger.debug(f"DC Check entailment, cn_B1 = {cn_B1}, cn_B2 = {cn_B2}")

    if cn_B1:
        kr_logger.info("DC: B1 does not entail alpha, calling DC recursively on B2")
        return shrink_divide_and_conquer(B1, alpha)

    if cn_B2:
        kr_logger.info("DC: B2 does not entail alpha, calling DC recursively on B1")
