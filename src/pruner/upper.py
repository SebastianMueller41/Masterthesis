...
class UpperPruner(BasePruner):
    ...
    def update_boundary_with_leaf(self, leaf_node):
        ...
        if leaf_path_measure > self.brancher.boundary:
            self.brancher.boundary = leaf_path_measure
            tree_logger.DEBUG(f"Updated boundary: {self.brancher.boundary}")
            print(f"Updated boundary: {self.brancher.boundary}")

    def should_prune(self, node):
        if self.brancher.boundary == 0:
            return False
        return self.calculate_potential_bound(node) <= self.brancher.boundary