...
class Brancher(BaseBrancher):
    ...
    def initialize_queue(self, pruner):
        if pruner == 'UPPER':
            ...
            self.queue = SortedList(key=lambda x: (-x[0], x[3]))  # Sort desc by sub_value and asc by cardinality
        elif pruner == 'LOWER':
    ...
    def expand_children(self, current_node):
        for element in current_node.get_kernel():
            ...
            self.tree_logger.debug(f"Adding node element {element} to queue with sub_value: {sub_value}")

    def add_to_priority_queue(self, node):
        cardinality = len(self.tree.get_hitting_set_for_leaf(node).get_elements())
        tree_logger.debug(f"Adding node to queue with: Subvalue {node.sub_value}, path_value {node.path_value}, card {cardinality}")
        self.queue.add((node.sub_value, node.path_value, node, cardinality))
