...
from src.kernels.kernelandremainder import KernelAndRemainderFinder  # Update import statement
...
if __name__ == "__main__":
    ...
    kernel_remainder_finder = KernelAndRemainderFinder(args.sw_size, args.divide_conquer)

    try:
        if args.method == 'kernel':
            if args.strategy_param == 0:
                hitting_set_tree = KernelSolver(BFS(kernel_remainder_finder, DataSet(args.dataset_file, args.strategy_param), args.alpha)).solve()
            elif 0 < args.strategy_param < 4:
                hitting_set_tree = KernelSolver(HybridSearch(kernel_remainder_finder, DataSet(args.dataset_file, args.strategy_param), args.alpha, args.strategy_param)).solve()
            else:
